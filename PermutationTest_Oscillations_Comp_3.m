...
for i = 1:length(subjects)
    ...
    filename1 = strcat('PROCESSED_DATA_NIKITA\',  num2str(subjects(i)), '_trial_sel_comp_3_a');
    ...
    Condition1{i} = ft_freqanalysis(cfg, dummy.data_tar_not_learned_2);
    clear dummy
end
...
for i = 1:length(subjects)
    ...
    filename2 = strcat('PROCESSED_DATA_NIKITA\',num2str(subjects(i)), '_trial_sel_comp_3_b'); %Learned versus not learned targets during first exposure; learned targets
    dummy2 = load(filename2);
    Condition2{i} = ft_freqanalysis(cfg, dummy2.data_fill_matched_not_learned);
    clear dummy2
end
